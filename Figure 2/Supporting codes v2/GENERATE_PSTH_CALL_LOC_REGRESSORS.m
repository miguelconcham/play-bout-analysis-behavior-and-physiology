function psth_struct = GENERATE_PSTH_CALL_LOC_REGRESSORS(animal_code, pt, f, freq_range, wind_length, spect_bin_size)
% Computes dual-band LFP power PSTHs around play bouts with kinematic regressors.
...
low_wind_length  = wind_length{1};
low_wind_overlap = low_wind_length - spect_bin_size;
high_wind_length  = wind_length{2};
...
%% Compute spectrograms and peri-event PSTHs with regressors
...
for ch_n = 1:numel(mid_PAG_channel)
    ...
    n_bins = round(range(hist_range) / spect_bin_size);
