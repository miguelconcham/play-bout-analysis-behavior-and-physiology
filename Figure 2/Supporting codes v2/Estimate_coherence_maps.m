%% Estimate_coherence_maps
% Driver script: compute PLI/coherence maps between brain areas (PAG recordings).
% Calls GENERATE_COHERNECE_MAPS_STRUCT (note: v2 GENERATE not yet in repo).
% Subsequent sections load, merge, and plot coherence structures from multiple sources.
...
%% Estimate coherence maps
% NOTE: GENERATE_COHERNECE_MAPS_STRUCT is not yet available in Supporting codes v2.
pli_struct = [];
animal_names = {};

for fn = 1:numel(animal_list)
    animal_path = fullfile(npx_Raw_Data, animal_list(fn).name);
    new_struct = GENERATE_COHERNECE_MAPS_STRUCT(animal_path, Hd_freq1, Hd_freq2, 'PAG');

    if isempty(pli_struct)
        pli_struct = new_struct;
    else
        start_idx = numel(pli_struct) + 1;
        for sub_j = 1:numel(new_struct)
            pli_struct(start_idx + sub_j - 1) = new_struct(sub_j);
        end
    end
...
pli_struct_NPX2 = pli_struct;
clear pli_struct

%% Save results
disp('saving')
save(fullfile(saving_folder, 'coherence_structure_pli_struct_NPX2.mat'), 'pli_struct_NPX2', '-v7.3');
save(fullfile(saving_folder, 'coherence_structure_animal_names_pli_struct_NPX2.mat'), 'animal_names');

%% Load and merge coherence structures from multiple sources
disp('loading')
load(fullfile(saving_folder, 'coherence_structure_pli_struct_mpfc.mat'), 'pli_struct_mpfc');
load(fullfile(saving_folder, 'coherence_structure_pli_struct_B1D11007.mat'), 'pli_struct_B1D11007');
load(fullfile(saving_folder, 'coherence_structure.mat'), 'pli_struct');
load(fullfile(saving_folder, 'coherence_structure_animal_names.mat'), 'animal_names');
load(fullfile(saving_folder, 'coherence_structure_pli_struct_NPX2.mat'), 'pli_struct_NPX2');
