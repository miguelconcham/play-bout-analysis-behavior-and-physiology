function psth_struct = GENERATE_PSTH_SPECTROGRAM(current_dir, wind_length, wind_overlap, min_separation, f, freq_pow_range)
...
%% Compute full-spectrogram PSTH around play bouts
hist_range       = [-20 20];
range_time_wrap  = [-5 5];
...
for ch_n = 1:numel(mid_PAG_channel)
    ...
    play_bout_tw_this = nan(size(play_bouts_table, 1), size(pow_spectrogram, 1), round(range(range_time_wrap) / spect_bin_size) + n_bins_time_wrap);
