...
%% Estimate PSTH area maps across animals
psth_structure = [];
animal_names = {};

for fn = 1:numel(animal_list)
    animal_path = fullfile(npx_Raw_Data, animal_list(fn).name);
    new_struct = GENERATE_PSTH_AREA_MAPS(animal_path, wind_length, wind_overlap, min_separation, f, freq_pow_range);
...
% disp('saving')
% save(fullfile(saving_folder, 'psth_structure_theta_map_corrected_files.mat'), 'psth_structure', '-v7.3');
% save(fullfile(saving_folder, 'animal_names_theta_map_corrected_files.mat'), 'animal_names');
