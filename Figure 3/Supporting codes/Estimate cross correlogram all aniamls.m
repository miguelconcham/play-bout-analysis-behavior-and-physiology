...
for fn = 1:numel(animal_list)
    ...
    if fn==1
        cross_corr_struct  = GENERATE_CROSS_CORR_STRUCTURE([npx_Raw_Data, '\', animal_list(fn).name],bin_size, hist_range);     
        ...
        n_strctut = n_strctut+numel(cross_corr_struct);
        animal_names = [animal_names;[repmat(animal_list(fn).name,numel(cross_corr_struct),1) num2cell(1:numel(cross_corr_struct))']];
    else
       transt_psth  = GENERATE_CROSS_CORR_STRUCTURE([npx_Raw_Data, '\', animal_list(fn).name],bin_size, hist_range);
        % save([saving_folder,'\',animal_list(fn).name,'_FreqRange_',num2str([Hd_freq.CutoffFrequency1 Hd_freq.CutoffFrequency2])...
        %     ,'_phase_couplig_structure.mat'],'transt_psth', '-v7.3');

        for sub_j=1:numel(transt_psth)

            cross_corr_struct(n_strctut) = transt_psth(sub_j);
...
all_area_comb                   = [all_area_comb;cross_corr_struct(fn).these_neurons_areas(cross_corr_struct(fn).cross_corr_comb)];
session_id                      = [session_id;repmat(animal_names(fn,1),size(cross_corr_struct(fn).cross_corr_comb,1),1)];
clusters_id = [clusters_id;cross_corr_struct(fn).cluster_info.cluster_id(cross_corr_struct(fn).cross_corr_comb)];
...
smoth_wind = 2;
pre_wind = [-.010 0];
pre_wind_index = time_centers>pre_wind(1) & time_centers>pre_wind(2);
post_wind = [0 .010];
post_wind_index = time_centers>post_wind(1) & time_centers>post_wind(2);
y_lim = [0.0022 0.0028];
n2plot = 100;
n_plot = 1;
cc2use =1;
 for j=1:size(unique_area_combinations,1)
    ...
    pre_responses_play = mean(this_cc(:,pre_wind_index),2);
    post_responses_play = mean(this_cc(:,post_wind_index),2);
     this_cc = squeeze(non_play_cc(area_combination_indexes,:));     
      ...
      this_cc = diag(1./sum(this_cc,2))*this_cc;
      pre_responses_non_play = mean(this_cc(:,pre_wind_index),2);
    post_responses_non_play = mean(this_cc(:,post_wind_index),2);
    X = [pre_responses_non_play*0 post_responses_non_play*0+1 pre_responses_play*0+2 post_responses_play*0+3]' ...
           + .25*(rand(size(pre_responses_non_play,1),4)-.5)';
    Y =[pre_responses_non_play post_responses_non_play pre_responses_play post_responses_play]';
    rand_selection= randsample(size(X,2),min(n2plot,size(X,2)));

       plot( X([1 2],rand_selection),...
          Y([1 2],rand_selection),':k')
       hold on
         plot( X([1 2],rand_selection),...
          Y([1 2],rand_selection),'.k', 'MarkerSize',8)

         plot( X([3 4],rand_selection),...
          Y([3 4],rand_selection),':k')

         plot( X([3 4],rand_selection),...
          Y([3 4],rand_selection),'.k', 'MarkerSize',8)
         play_p = signrank(pre_responses_play,post_responses_play);
          non_play_p = signrank(pre_responses_non_play,post_responses_non_play);
       plot(median(X,2, 'omitmissing'), median(Y,2, 'omitmissing'), 'r_', 'MarkerSize',8)
       plot(median(X,2, 'omitmissing'), median(Y,2, 'omitmissing'), 'r', 'LineWidth',2)
       ylim(y_lim)
     title({[unique_area_combinations{j,1}{1}, ' to ' ,unique_area_combinations{j,2}{1}], num2str([play_p non_play_p])})

n_plot = n_plot+1;

 end
%% same but onsqure pltos

time_centers = .5*(psth_edges(1:end-1)+psth_edges(2:end));
 figure('units','normalized','outerposition',[0  0 .75 .75]);
smoth_wind = 2;
pre_wind = [-.010 0];
pre_wind_index = time_centers>pre_wind(1) & time_centers>pre_wind(2);
post_wind = [0 .010];
...
n_plot = 1;
 for j=1:size(unique_area_combinations,1)
     ...
     area_combination_indexes = ismember(area_combinations.Neuron1,unique_area_combinations.Neuron1(j)) & ismember(area_combinations.Neuron2,unique_area_combinations.Neuron2(j));
     hold on
     this_cc = squeeze(play_cc(cc2use,area_combination_indexes,:));
     for k = 1:size(this_cc,1)
         this_cc(k,:) = movmean( this_cc(k,:),smoth_wind);
     end
     this_cc = diag(1./sum(this_cc,2))*this_cc;
    pre_responses_play = mean(this_cc(:,pre_wind_index),2);
    post_responses_play = mean(this_cc(:,post_wind_index),2);
     this_cc = squeeze(non_play_cc(cc2use,area_combination_indexes,:));     
 ...
 play_p = signrank(pre_responses_play,post_responses_play);
    ...
    rand_selection= randsample(size(X,2),min(n2plot,size(X,2)));
       ...
       loglog( Y(1,rand_selection),...
          Y(2,rand_selection),'.k')

       hold on
       loglog(mean( Y(1,rand_selection), 'omitmissing'),...
          mean(Y(2,rand_selection), 'omitmissing'),'Xb')
       loglog(y_lim,y_lim,'r')
        
       axis([y_lim y_lim])
        title({['centered in' unique_area_combinations{j,1}{1}], ['non play ', num2str( non_play_p)]})
        xlabel(['Pre ', unique_area_combinations{j,2}{1}])
        ylabel(['Post ', unique_area_combinations{j,2}{1}])
          subplot(2,5,n_plot+5)
     loglog( Y(3,rand_selection),...
          Y(4,rand_selection),'.k')
      hold on
       loglog(mean( Y(3,rand_selection), 'omitmissing'),...
          mean(Y(4,rand_selection), 'omitmissing'),'Xb')
       loglog(y_lim,y_lim,'r')
        
       axis([y_lim y_lim])
       title({['centered in' unique_area_combinations{j,1}{1}], ['play ', num2str(play_p)]})
        xlabel(['Pre ', unique_area_combinations{j,2}{1}])
        ylabel(['Post ', unique_area_combinations{j,2}{1}])
...
figure
 for j=1:size(unique_area_combinations,1)
     ...
     this_cc = squeeze(all_cross_corr_norm(area_combination_indexes,:));
     for k = 1:size(this_cc,1)
         this_cc(k,:) = movmean( this_cc(k,:),smoth_wind);
     end
     this_cc = diag(1./sum(this_cc,2))*this_cc;
 ...
 hold on
     ...
     xlim([-.1 .1])
       ...
       % yyaxis right
       hold on
     this_cc = squeeze(play_cc(1,area_combination_indexes,:));
     for k = 1:size(this_cc,1)
         this_cc(k,:) = movmean( this_cc(k,:),smoth_wind);
     end
     this_cc = diag(1./sum(this_cc,2))*this_cc;
     ...
     fill([time_centers(no_nan) fliplr(time_centers(no_nan))],[ci(1,no_nan) fliplr(ci(2,no_nan))],'r', 'FaceAlpha',.25, 'EdgeColor','none')
     plot(time_centers,mean(this_cc, 'omitmissing'), 'r')
     this_cc = squeeze(non_play_cc(1,area_combination_indexes ,:));     
...
%% defien trough peak and non entrained categorires
...
plot_logit = ~plot_ci;
 ...
 %% prepare dat for a specific area to area interaction  (j=14 is LPG vs SupCol)
 ...
 smoth_wind = 1;
 area_combination_indexes = ismember(area_combinations.Neuron1,unique_area_combinations.Neuron1(j)) & ismember(area_combinations.Neuron2,unique_area_combinations.Neuron2(j));
 cell_type_comb_trough          = find(trough(idx_pairs(:,1)) &  trough(idx_pairs(:,2)) & area_combination_indexes);
 cell_type_comb_non_entrained   = find(non_entrained(idx_pairs(:,1)) & non_entrained(idx_pairs(:,2))  & area_combination_indexes) ;
 cell_type_comb_peak            = find(peak(idx_pairs(:,1)) &  peak(idx_pairs(:,2))  & area_combination_indexes);
...
%Non entrained cells
 ...
 %% plot singe cross corr


 raw_n = 500;
 raw_n = 10;
x_lim = [-.5 .5];
bin_size = mean(diff(time_centers));
 
 figure('units','normalized','outerposition',[0 0 .3 1]);
 for kk=1:10
     subplot(5,2,kk)
    index1 = find(all_neurons_TD.cluster_id==clusters_id(cell_type_comb_trough(raw_n+kk-1),1) & ismember(all_neurons_TD.session,session_id{cell_type_comb_trough(raw_n+kk-1)}));
    index2 = find(all_neurons_TD.cluster_id==clusters_id(cell_type_comb_trough(raw_n+kk-1),2) & ismember(all_neurons_TD.session,session_id{cell_type_comb_trough(raw_n+kk-1)}));
...
sgtitle('Torugh cells')
 ...
 for kk=1:10
     subplot(5,2,kk)
      index1 = find(all_neurons_TD.cluster_id==clusters_id(cell_type_comb_peak(raw_n+kk-1),1) & ismember(all_neurons_TD.session,session_id{cell_type_comb_peak(raw_n+kk-1)}));
    index2 = find(all_neurons_TD.cluster_id==clusters_id(cell_type_comb_peak(raw_n+kk-1),2) & ismember(all_neurons_TD.session,session_id{cell_type_comb_peak(raw_n+kk-1)}));
    if ~isempty(index1) &  ~isempty(index2)
...
sgtitle('Peak cells')
 ...
 for kk=1:10
     subplot(5,2,kk)
     index1 = find(all_neurons_TD.cluster_id==clusters_id(cell_type_comb_non_entrained(raw_n+kk-1),1) & ismember(all_neurons_TD.session,session_id{cell_type_comb_non_entrained(raw_n+kk-1)}));
    index2 = find(all_neurons_TD.cluster_id==clusters_id(cell_type_comb_non_entrained(raw_n+kk-1),2) & ismember(all_neurons_TD.session,session_id{cell_type_comb_non_entrained(raw_n+kk-1)}));
    if ~isempty(index1) &  ~isempty(index2)
...
%% testing differnece betwen selected area (j=14 is LPG vs SupCol)
...
%% testing differnece betwen selected area now for non-antraiend cells (j=14 is LPG vs SupCol)
...
%% PLOTING ALL AREA COMBINATION TOGETHER %%
...
%% here below is for find an example %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%here below is for find an example%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
bin_size = mean(diff(time_centers));
 ...
 for kk_n=1:numel(list_of_neuron)
     ...
     subplot(5,5,nsp)
    index1 = find(all_neurons_TD.cluster_id==clusters_id(cell_type_comb_trough(kk),1) & ismember(all_neurons_TD.session,session_id{cell_type_comb_trough(kk)}));
    index2 = find(all_neurons_TD.cluster_id==clusters_id(cell_type_comb_trough(kk),2) & ismember(all_neurons_TD.session,session_id{cell_type_comb_trough(kk)}));
    % all_neurons_TD([index1,index2],:)
    if ~isempty(index1) &  ~isempty(index2)
...
%% select animal session and neuron id to plot
...
psth_edges = hist_range(1):bin_size:hist_range(2);
    ...
    GENERATE_CROSS_CORR_EXAMPLE([npx_Raw_Data, '\', animal_list.name],bin_size, hist_range,[id_1 id_2],time_precision);
