function synch_structure = GENERATE_SURPIRSE_DYNAMICS_STATS(npx_data_dir, bin_size, hist_range, time_precision, areas2compare)
...
nperm               = 2000;
smoth_wind_sec      = 0.05;
jitter_range        =time_precision +  [0 time_precision]; %% spikes shited [-jitter_range(2) -jitter_range(1)] U [jitter_range(1) jitter_range(2)], i.e outisde synchronization range
...
for n_comb = 1:size(synch_spikes_histogram,2)
    ...
    for pb_n=1:size(play_bouts_table,1)
        ...
        this_pb_spikes_2    = spikes_cluseter_2(spikes_cluseter_2>=pb_start+hist_range(1) & spikes_cluseter_2<=pb_start+hist_range(2));
        if rand<0.5
            direction = (2*(rand(size(this_pb_spikes_1))>.5) -1);
            this_pb_spikes_shifted_1    = this_pb_spikes_1 + direction.*(rand(size(this_pb_spikes_1))*range(jitter_range) +jitter_range(1)) ;
            ...
            direction = (2*(rand(size(this_pb_spikes_2))>.5) -1);
            this_pb_spikes_shifted_2    = this_pb_spikes_2 + direction.*(rand(size(this_pb_spikes_2))*range(jitter_range) +jitter_range(1)) ;
...
synch_structure.jitter_multiplier                = jitter_multiplier;
