...
for fn = 1:numel(animal_list)
    ...
    if fn==1
        synch_structure = GENERATE_SURPIRSE_DYNAMICS([npx_Raw_Data, '\', animal_list(fn).name],bin_size, hist_range,time_precision);
        ...
        transt_psth  = GENERATE_SURPIRSE_DYNAMICS([npx_Raw_Data, '\', animal_list(fn).name],bin_size, hist_range,time_precision);
...
% disp('saving')
% save([saving_folder,'\surpirse_struct_structure_no_play.mat'],'synch_structure', '-v7.3');
% save([saving_folder,'\surpirse_struct_animal_names_no_play.mat'],'animal_names');

%%
saving_folder = '\\experimentfs.bccn-berlin.pri\experiment\PlayNeuralData\NPX-OPTO PLAY NMM\PlayBout Analysis\play bout analysis behavior and physiology\Data\Analysis results\Cross_correlogram';
figure_folder = '\\experimentfs.bccn-berlin.pri\experiment\PlayNeuralData\NPX-OPTO PLAY NMM\PlayBout Analysis\play bout analysis behavior and physiology\Data\Figure codes\Figure cross correlations';

load([saving_folder,'\surpirse_struct_structure.mat'],'synch_structure');
load([saving_folder,'\surpirse_struct_animal_names.mat'],'animal_names');
synch_structure(2) = [];
...
for fn=1:numel(corrected_synch_structure)
    ...
    % INPUTS
    % T      : n × m neuron metadata table
    % pairs  : N × 2 array of neuron pairs [j i], i > j
    % xcorrs : 2 × N × 600 cross-correlogram array
    T       = corrected_synch_structure(fn).cluster_info;
    pairs   = corrected_synch_structure(fn).cross_corr_comb;
    x1      = corrected_synch_structure(fn).synch_spikes_histogram;
    x2      = corrected_synch_structure(fn).desynch_spikes_histogram;
    x3      = corrected_synch_structure(fn).shifted_spikes_histogram;
    areas   = corrected_synch_structure(fn).these_neurons_areas;

    % --- 1. Keep only 'mua' and 'good' neurons
    keepNeuron = ismember(T.group, {'mua','good'});
    areas = areas(keepNeuron);

    % --- 2. Map old neuron indices -> new indices
    old2new = zeros(height(T),1);
    old2new(keepNeuron) = 1:nnz(keepNeuron);

    % --- 3. Filter neuron table
    T = T(keepNeuron,:);

    % --- 4. Keep only pairs where BOTH neurons survive
    pairMask = keepNeuron(pairs(:,1)) & keepNeuron(pairs(:,2));

    % --- 5. Filter pairs and correlograms
    pairs  = pairs(pairMask,:);
    x1 = x1(:, pairMask, :);
    x2 = x2(:, pairMask, :);
    x3 = x3(:, pairMask, :);

    % --- 6. Remap pair indices to new table indexing
    pairs(:,1) = old2new(pairs(:,1));
    pairs(:,2) = old2new(pairs(:,2));

    % --- 7. Sanity checks (optional but recommended)
    assert(all(pairs(:) > 0), 'Zero index found in pairs');
    assert(max(pairs(:)) == height(T), 'Index exceeds table size');
    assert(size(x1,2) == size(pairs,1), 'Pair / xcorr mismatch');



    corrected_synch_structure(fn).cluster_info              = T;
    corrected_synch_structure(fn).cross_corr_comb           = pairs;
    corrected_synch_structure(fn).synch_spikes_histogram    = x1;
    corrected_synch_structure(fn).desynch_spikes_histogram  = x2;
    corrected_synch_structure(fn).shifted_spikes_histogram  = x3;
    corrected_synch_structure(fn).these_neurons_areas      = areas;
end

%%
synch_spikes_histogram = [];
desynch_spikes_histogram = [];
...
for fn = 1:numel(corrected_synch_structure)
    ...
    desynch_spikes_histogram        = cat(2,desynch_spikes_histogram,corrected_synch_structure(fn).desynch_spikes_histogram);
    shifted_spikes_histogram        = cat(2,shifted_spikes_histogram,corrected_synch_structure(fn).shifted_spikes_histogram);
    all_area_comb                   = [all_area_comb;corrected_synch_structure(fn).these_neurons_areas(corrected_synch_structure(fn).cross_corr_comb)];
    session_id                      = [session_id;repmat(animal_names(fn,1),size(corrected_synch_structure(fn).cross_corr_comb,1),1)];
    clusters_id                     = [clusters_id;corrected_synch_structure(fn).cluster_info.cluster_id(corrected_synch_structure(fn).cross_corr_comb)];
...
for j=1:size(unique_area_combinations,1)
    ...
    if size(this_cc(index,:),1)>0
        if parametric_cond
            increased_surprise = nan(1,size(surprise,2));
            for t_index = 1:numel(increased_surprise)
                increased_surprise(t_index) = signrank(surprise(index,t_index));
            end
            start_end_sign = [find(diff([0 increased_surprise<alpha_level 0])==1)'  (find(diff([0 increased_surprise<alpha_level 0])==-1)-1)'];
            for se=1:size(start_end_sign,1)
                fill([time_centers(start_end_sign(se,:)) fliplr(time_centers(start_end_sign(se,:)))],[this_ylim([2 2]) this_ylim([2 2])*1.1],'r', 'FaceAlpha',.2, 'EdgeColor','none')
            end
        else
            [~, ~, ci] = ttest(surprise(index,:), 0,'alpha',alpha_level);

            [~, increased_surprise] = ttest(surprise(index,:), 0,'alpha',0.05);
            no_nan = ~any(isnan(ci(:,2)));
            fill([time_centers fliplr(time_centers)],[ci(1,:) fliplr(ci(2,:))],plot_color, 'FaceAlpha',.25, 'EdgeColor','none')
    ...
    set(gca, 'TickDir', 'out')
    title(unique_area_combinations.Neuron1(j))
...
total_pval_periods           = nan(size(unique_area_combinations,1),3);;
...
for j=1:size(unique_area_combinations,1)
    ...
    d = mu ./ sd;
     ...
     effect_size{j,cell_type} = d(time_centers>=x_lim(1) & time_centers<=x_lim(2));
        ...
        [L,n]= bwlabeln(h);
        total_pval_periods(j,cell_type)  = sum(h);
...
subplot(3,1,1)
plot([0 numel(idx)+1], [0.95 0.95], ':k')
hold on
bar(1- (p_Val_per_cell(idx,1)*15), 'b')
xticks(1:15)
...
plot([0 numel(idx)+1], [0.95 0.95], ':k')
hold on
bar(1- (p_Val_per_cell(idx,2)*15), 'r')
ylim([0.90 1])
...
plot([0 numel(idx)+1], [0.95 0.95], ':k')
hold on
bar(1- (p_Val_per_cell(idx,3)*15), 'g')
xticks(1:15)
xticklabels(legnedfor_xticks)
ylim([0.90 1])
...
subplot(3,1,1)
plot([0 numel(idx)+1], [0.02 0.02], ':k')
hold on
bar(p_Val_length_per_cell(idx,1)*bin_length, 'b')
xticks(1:15)
xticklabels(legnedfor_xticks)
ylim([0 .15])
...
plot([0 numel(idx)+1], [0.02 0.02], ':k')
hold on
bar(p_Val_length_per_cell(idx,2)*bin_length, 'r')
ylim([0 .15])
...
plot([0 numel(idx)+1], [0.02 0.02], ':k')
hold on
bar(p_Val_length_per_cell(idx,3)*bin_length, 'g')
xticks(1:15)
xticklabels(legnedfor_xticks)
ylim([0 .15])
...
plot([0 numel(idx)+1], [0.02 0.02], ':k')
hold on
bar(total_pval_periods(idx,1)*bin_length, 'b')
...
plot([0 numel(idx)+1], [0.02 0.02], ':k')
hold on
bar(total_pval_periods(idx,2)*bin_length, 'r')
ylim([0 .3])
...
plot([0 numel(idx)+1], [0.02 0.02], ':k')
hold on
bar(total_pval_periods(idx,3)*bin_length, 'g')
xticks(1:15)
xticklabels(legnedfor_xticks)
ylim([0 .3])
%%
