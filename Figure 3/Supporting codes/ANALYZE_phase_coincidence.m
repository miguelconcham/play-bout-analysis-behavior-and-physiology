...
animal_list(1:2) = [];
figure_folder = '\\experimentfs.bccn-berlin.pri\experiment\PlayNeuralData\NPX-OPTO PLAY NMM\PlayBout Analysis\DataSets\Codes\Figure codes\Figure coincident phases';
...
load([saving_folder,'\',animal_list(fn).name,'_FreqRange_',num2str([Hd_freq.CutoffFrequency1 Hd_freq.CutoffFrequency2])...
...
for j=1:size(unique_area_combinations,1)
   ...
   % subplot(2,1,1)
   % polarhistogram(synch_spikes_phases_after(trough_trough,1), -pi:(pi/16):pi, 'FaceColor','b', 'FaceAlpha',.5, 'EdgeColor','none', 'Normalization','percentage')

   %     hold on
   % 
   % 
   % 
   % polarhistogram(synch_spikes_phases_after(peak_peak,1), -pi:(pi/16):pi, 'FaceColor','r', 'FaceAlpha',.5, 'EdgeColor','none', 'Normalization','percentage')
   %  % polarhistogram(synch_spikes_phases_after(trough_peak,1), -pi:(pi/16):pi, 'FaceColor','g', 'FaceAlpha',.4, 'EdgeColor','none', 'Normalization','percentage')
   % rlim([0 20]);
   % r_lim = [0 20];
   %  mean_angle_trough = circ_mean(synch_spikes_phases_after(trough_trough & ~isnan(synch_spikes_phases_after(:,1)),1));
   %   polarplot([mean_angle_trough mean_angle_trough], r_lim, 'b', 'LineWidth',2)
   %  mean_angle_peak = circ_mean(synch_spikes_phases_after(peak_peak & ~isnan(synch_spikes_phases_after(:,1)),1));
   %  polarplot([mean_angle_peak mean_angle_peak], r_lim, 'r', 'LineWidth',2)
   %  % mean_angle_trough = circ_mean(synch_spikes_phases_after(trough_peak,1));
   % title('After')
   % % % subplot(2,1,2)
   % % polarhistogram(synch_spikes_phases_before(trough_trough,1), -pi:(pi/16):pi, 'FaceColor','b', 'FaceAlpha',.8, 'EdgeColor','none', 'Normalization','percentage')
   % % hold on
   % % polarhistogram(synch_spikes_phases_before(peak_peak,1), -pi:(pi/16):pi, 'FaceColor','r', 'FaceAlpha',.2, 'EdgeColor','none', 'Normalization','percentage')
   % %    polarhistogram(synch_spikes_phases_before(trough_peak,1), -pi:(pi/16):pi, 'FaceColor','g', 'FaceAlpha',.8, 'EdgeColor','none', 'Normalization','percentage')
   % % 
   % % title('Before')
   % print(gcf,'-vector','-dsvg',[figure_folder,'/',[unique_area_combinations.Neuron1{j}, ' ',unique_area_combinations.Neuron2{j}], ' distributuion of phases of coincidences.svg'])
   % pause(.1)
   ...
   % x_tick_labels{j}= [unique_area_combinations.Neuron1{j}, ' ',unique_area_combinations.Neuron2{j}];
   %  sgtitle([unique_area_combinations.Neuron1{j}, ' ',unique_area_combinations.Neuron2{j}])
end
%%
figure
circ_rtest(all_angles(all_angles(:,3)==2,1))
polarhistogram(all_angles(all_angles(:,3)==2,1), -pi:pi/16:pi,'FaceColor', 'r', 'Normalization','pdf')
hold on
polarhistogram(all_angles(all_angles(:,3)==1,1), -pi:pi/16:pi,'FaceColor', 'b', 'Normalization','pdf')
polarhistogram(all_angles(all_angles(:,3)==3,1), -pi:pi/16:pi,'FaceColor', 'g', 'Normalization','pdf')


%%
r_change = r_per_condition_after-r_per_condition_before;
r_change(counts_per_condition<20) = NaN;
[h,p,ci]=ttest(r_change);
figure
plot(r_change', ':k')
hold on
plot(r_change', '.k', 'MarkerSize',10)
%%
delta_mvl = mvl_before_after(:,2) - mvl_before_after(:,1);

figure('Color','w','Position',[100 100 800 1200])

for j = 1:size(unique_area_combinations,1)

    area_idx = ...
        (strcmp(area_combinations.Neuron1,unique_area_combinations.Neuron1{j}) & ...
         strcmp(area_combinations.Neuron2,unique_area_combinations.Neuron2{j})) | ...
        (strcmp(area_combinations.Neuron2,unique_area_combinations.Neuron1{j}) & ...
         strcmp(area_combinations.Neuron1,unique_area_combinations.Neuron2{j}));

    trough_trough = area_idx & ...
        trough(idx_pairs(:,1)) & ...
        trough(idx_pairs(:,2));
    ...
    non_entrained = area_idx & ...
        ( ...
        (trough(idx_pairs(:,1)) & peak(idx_pairs(:,2))) | ...
        (peak(idx_pairs(:,1)) & trough(idx_pairs(:,2))) ...
        );
...
hold on
    ...
    groups = {trough_trough, peak_peak, non_entrained};
    labels = {'TT','PP','TP'};

    for k = 1:3

        idx = groups{k} & all(~isnan(mvl_before_after),2);
        ...
        pvals(k) = signrank( ...
            mvl_before_after(idx,1), ...
            mvl_before_after(idx,2));
    ...
    title(sprintf('%s-%s   pTT=%.3g  pPP=%.3g  pTP=%.3g', ...
        unique_area_combinations.Neuron1{j}, ...
        unique_area_combinations.Neuron2{j}, ...
        pvals(1),pvals(2),pvals(3)))

    ylabel('\Delta MVL')
end

% sgtitle('After - Before coincidence-phase MVL')

%%

alpha_sig = 0.01;

nAreas = size(unique_area_combinations,1);

mean_diff = nan(nAreas,3);
pvals     = nan(nAreas,3);
counts    = zeros(nAreas,3);
...
for j = 1:nAreas
    ...
    for k = 1:3
        ...
        before = mvl_before_after(idx,1);
        after  = mvl_before_after(idx,2);

        mean_diff(j,k) = mean(after-before,'omitnan');

        pvals(j,k) = signrank(before,after);
...
%%
area_labels = strcat( ...
    unique_area_combinations.Neuron1 , ...
    "-", ...
    unique_area_combinations.Neuron2 );

figure('Color','w','Position',[100 100 1600 900])

group_names = {'Trough-Trough','Peak-Peak','Trough-Peak'};
group_colors = {'b','r','g'};
...
for k = 1:3
    ...
    for j = 1:nAreas
        ...
        if isnan(mean_diff(j,k))
            continue
        end

        if pvals(j,k) < alpha_sig

            bar(j, ...
                mean_diff(j,k), ...
                'FaceColor',group_colors{k}, ...
                'EdgeColor',group_colors{k});
    ...
    xticks(1:nAreas)
    xticklabels(area_labels)
    xtickangle(45)

    ylabel('\Delta MVL')

    title(sprintf('%s  (filled = p < %.3f)', ...
        group_names{k},alpha_sig))

    box off

end

sgtitle('Mean change in coincidence-phase MVL (After - Before)')
%%

alpha_sig = 0.01;

mean_mvl_after = nan(nAreas,3);
p_after = nan(nAreas,3);
for j = 1:nAreas
    ...
    for k = 1:3
        ...
        mean_mvl_after(j,k) = mean(after,'omitnan');

        p_after(j,k) = signrank(after);
...
for k = 1:3
    ...
    for j = 1:nAreas
        ...
        if isnan(mean_mvl_after(j,k))
            continue
        end

        if p_after(j,k) < alpha_sig
...
sgtitle('Phase locking of coincident spikes after play')
%%

figure
subplot(1,2,1)
b=bar(squeeze(r_per_condition_before));
b(1).FaceColor = [.2 .2 .8];
b(2).FaceColor = [.8 .2 .2];
b(3).FaceColor = [.8 .2 .8];
xticks(1:15)
...
subplot(1,2,2)
b=bar(squeeze(r_per_condition_after));
b(1).FaceColor = [.2 .2 .8];
b(2).FaceColor = [.8 .2 .2];
b(3).FaceColor = [.8 .2 .8];
xticks(1:15)
...
figure
subplot(1,2,1)
b=bar(squeeze(angle_per_condition_before)+pi);
b(1).FaceColor = [.2 .2 .8];
b(2).FaceColor = [.8 .2 .2];
b(3).FaceColor = [.8 .2 .8];
xticks(1:15)
...
subplot(1,2,2)
b=bar(squeeze(angle_per_condition_after)+pi);
b(1).FaceColor = [.2 .2 .8];
b(2).FaceColor = [.8 .2 .2];
b(3).FaceColor = [.8 .2 .8];
xticks(1:15)
...
%% mean angles per conditions
figure
for j=1:size(unique_area_combinations,1)

    subplot(5,3,j)
    polarplot([1 1]*angle_per_condition_before(j,1), [0 r_per_condition_before(j,1)],':b', 'LineWidth',3)
    hold on
    polarplot([1 1]*angle_per_condition_after(j,1),[0 r_per_condition_after(j,1)], 'b', 'LineWidth',3)

     polarplot([1 1]*angle_per_condition_before(j,2), [0 r_per_condition_before(j,1)],':r', 'LineWidth',3)
    polarplot([1 1]*angle_per_condition_after(j,2),[0 r_per_condition_after(j,1)], 'r', 'LineWidth',3)

    polarplot([1 1]*angle_per_condition_before(j,3), [0 r_per_condition_before(j,1)],':g', 'LineWidth',3)
    polarplot([1 1]*angle_per_condition_after(j,3),[0 r_per_condition_after(j,1)], 'g', 'LineWidth',3)
    rlim([0 .6])
    title([unique_area_combinations.Neuron1{j}, ' ',unique_area_combinations.Neuron2{j}, ' #', num2str(counts_per_condition(j,1)), '/', num2str(counts_per_condition(j,2)), '/',num2str(counts_per_condition(j,3))])
end
...
r_per_condition_diff      = nan(size(unique_area_combinations,1),3);
angle_per_condition_diff= nan(size(unique_area_combinations,1),3);
is_it_centered = nan(size(unique_area_combinations,1),3);
...
for j=1:size(unique_area_combinations,1)
   ...
   non_entrained = ( ( peak(idx_pairs(:,1)) &  trough(idx_pairs(:,2)) | (trough(idx_pairs(:,1)) &  peak(idx_pairs(:,2))) ) & area_combination_indexes  & mean(cluster_fr,2)>min_fr);
   ...
   if ~isempty(trough_trough)
       ...
       % Test if the mean difference is 0
       if ~isempty(diff)
            polarhistogram(diff, -pi:(pi/16):pi, 'FaceColor','b', 'FaceAlpha',.5, 'EdgeColor','none', 'Normalization','percentage')
            r_per_condition_diff(j,1) = circ_r(diff);
            angle_per_condition_diff(j,1) = circ_mean(diff);

            is_it_centered(j,1) = circ_rtest(diff);
            hold on
           [pval, z] = circ_mtest(diff, 0, alpha_level);
           p_val_before_after_condition(j,1) = pval;
       end
   ...
   if ~isempty(peak_peak)
       ...
       diff = circ_dist(alpha_before, alpha_after);
       if ~isempty(diff)
              polarhistogram(diff, -pi:(pi/16):pi, 'FaceColor','r', 'FaceAlpha',.5, 'EdgeColor','none', 'Normalization','percentage')
               r_per_condition_diff(j,2) = circ_r(diff);
            angle_per_condition_diff(j,2) = circ_mean(diff);
            is_it_centered(j,2) = circ_rtest(diff);
                 hold on
           % Test if the mean difference is 0
           [pval, z] = circ_mtest(diff, 0, alpha_level);
           p_val_before_after_condition(j,2) = pval;
   ...
   if ~isempty(non_entrained)
       ...
       diff = circ_dist(alpha_before, alpha_after);
       if ~isempty(diff)
            polarhistogram(diff, -pi:(pi/16):pi, 'FaceColor','g', 'FaceAlpha',.5, 'EdgeColor','none', 'Normalization','percentage')
             r_per_condition_diff(j,3) = circ_r(diff);
            angle_per_condition_diff(j,3) = circ_mean(diff);
            is_it_centered(j,3) = circ_rtest(diff);
           % Test if the mean difference is 0
           [pval, z] = circ_mtest(diff, 0, alpha_level);
           p_val_before_after_condition(j,3) = pval;
       end
...
alpha_level = 0.05;
figure
group_labels = cell(size(unique_area_combinations,1),1);
for j=1:size(unique_area_combinations,1)

    subplot(5,3,j)
    if is_it_centered(j,1)<alpha_level
        polarplot([1 1]*angle_per_condition_diff(j,1),[0 r_per_condition_diff(j,1)], 'b', 'LineWidth',3)
    else
        polarplot([1 1]*angle_per_condition_diff(j,1),[0 r_per_condition_diff(j,1)], ':b', 'LineWidth',3)
    end
    hold on
    if is_it_centered(j,2)<alpha_level
        polarplot([1 1]*angle_per_condition_diff(j,2),[0 r_per_condition_diff(j,2)], 'r', 'LineWidth',3)
    else
        polarplot([1 1]*angle_per_condition_diff(j,2),[0 r_per_condition_diff(j,2)], ':r', 'LineWidth',3)
    end
    if is_it_centered(j,3)<alpha_level
        polarplot([1 1]*angle_per_condition_diff(j,3),[0 r_per_condition_diff(j,3)], 'g', 'LineWidth',3)
    else
        polarplot([1 1]*angle_per_condition_diff(j,3),[0 r_per_condition_diff(j,3)], ':g', 'LineWidth',3)
    end
    rlim([0 .3])
    group_labels{j} = [unique_area_combinations.Neuron1{j}, ' ',unique_area_combinations.Neuron2{j}];
    title([unique_area_combinations.Neuron1{j}, ' ',unique_area_combinations.Neuron2{j}, ' #', num2str(counts_per_condition(j,1))])
end
...
%% Sinusoidal Plot (Corrected Alignment & Staggering)
