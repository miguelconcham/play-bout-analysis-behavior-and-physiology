function synch_structure = GENERATE_SURPIRSE_DYNAMICS(npx_data_dir, bin_size, hist_range, time_precision)
...
synch_spikes_histogram          = nan(2,size(cross_corr_comb,1),numel(psth_edges)-1);
desynch_spikes_histogram        = nan(2,size(cross_corr_comb,1),numel(psth_edges)-1);
shifted_spikes_histogram        = nan(2,size(cross_corr_comb,1),numel(psth_edges)-1);
...
for n_comb = 1:size(synch_spikes_histogram,2)
    ...
    synch_spikes_histogram_shifted_n1 = nan(size(play_bouts_table,1),numel(psth_edges)-1);
    synch_spikes_histogram_shifted_n2 = nan(size(play_bouts_table,1),numel(psth_edges)-1);
    ...
    for pb_n=1:size(play_bouts_table,1)
        ...
        if rand<0.5
        this_pb_spikes_shifted_1    = this_pb_spikes_1 + 2*(rand(size(this_pb_spikes_1)) -.5)*time_precision*jitter_multiplier ;
        ...
        this_pb_spikes_shifted_2    = this_pb_spikes_2 +2*(rand(size(this_pb_spikes_2))-.5)*time_precision*jitter_multiplier;
        ...
        if ~isempty(this_pb_spikes_2) & ~isempty(this_pb_spikes_1)
            ...
            synch_spikes_histogram_shifted_n1(pb_n,:) = histcounts(this_pb_spikes_1(s1_index)-pb_start,psth_edges);
            synch_spikes_histogram_shifted_n2(pb_n,:) = histcounts(this_pb_spikes_2(s2_index)-pb_start,psth_edges);
        end
    ...
    shifted_spikes_histogram(1,n_comb,:) = mean(synch_spikes_histogram_shifted_n1, 'omitmissing');
    shifted_spikes_histogram(2,n_comb,:) = mean(synch_spikes_histogram_shifted_n2, 'omitmissing');
    ...
    desynch_spikes_histogram(1,n_comb,:) = mean(desynch_spikes_histogram_n1, 'omitmissing');
    desynch_spikes_histogram(2,n_comb,:) = mean(desynch_spikes_histogram_n2, 'omitmissing');
...
synch_structure.desynch_spikes_histogram        = desynch_spikes_histogram;
synch_structure.synch_spikes_histogram          = synch_spikes_histogram;
synch_structure.shifted_spikes_histogram        = shifted_spikes_histogram;
...
synch_structure.cross_corr_comb                 = cross_corr_comb;
